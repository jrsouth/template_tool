...
/*
 * Inline version: MakeFont() altered to accept full pathname for output
 *                 Message output suppressed except errors
 *                 Lack of gzip considered an error
 */
...
{
if ($severity !== 'Error' ) return; // Skip all non-error output
...
function MakeFont($fontfile, $outputPath='./', $enc='cp1252', $embed=true)
{
	...
	$basename = $outputPath . substr(basename($fontfile), 0, -4);
	if($embed)
	{
		...
		else
		{
			...
			Error('Font file could not be compressed (zlib extension not available)');
		}
	}
